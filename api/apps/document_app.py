...
async def get(doc_id):
    try:
        ...
        if data is not None:
            _orig_name = str(n).split("/")[-1]
            _rotated_n = f"{doc_id}/{re.sub(r'(?i)\.pdf$', '_rotated.pdf', _orig_name)}"
            if _rotated_n != str(n):
                try:
                    _rotated_data = await asyncio.to_thread(
                        settings.STORAGE_IMPL.get, b, _rotated_n
                    )
                    if _rotated_data is not None and len(_rotated_data) > 0:
                        data = _rotated_data
                        logging.info("[MinerU] 使用旋转修正版 PDF 供前端展示: %s/%s", b, _rotated_n)
                except Exception:
...
async def batch_doc_progress(tenant_id):
    ...
    try:
        for doc_id in normalized_doc_ids:
            e, doc = DocumentService.get_by_id(doc_id)
            if not e or not doc:
                progress_list.append({"doc_id": doc_id, "fileId": doc_id, "progress": 0.0, "name": "", "type": "", "doc_type_en": "", "doc_type_cn": ""})
                continue
            ...
            progress_list.append({
                ...
                "doc_type_en": doc.doc_type_en if getattr(doc, "doc_type_en", None) is not None else "",
                "doc_type_cn": doc.doc_type_cn if getattr(doc, "doc_type_cn", None) is not None else "",
            })
