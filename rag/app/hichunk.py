...
def chunk(filename, binary=None, lang="Chinese", callback=None, **kwargs):
    ...
    if is_mineru_doc or is_mineru_img:
        ...
        _mineru_ok, _mineru_reason = mineru_parser.check_installation(backend=backend, server_url=_mineru_srv)  # 解析前探活，返回(bool,说明)
        if not _mineru_ok:  # 探活失败不中断： openapi 探测失败不等于 file_parse 不可用
            logging.warning("[MinerU][hichunk] 解析前 API 探活未通过(%s)，仍继续 parse_document。", _mineru_reason)
            if callback:  # 向任务进度条提示仍可继续解析
    ...
    def _hichunk_merge_sort_key_when_all_indexed(elem):
        mi = elem['mineru_index']
        seq = elem.get('chunk_seq', 0)
        if elem['type'] == 'table':
            return (1, mi, seq)
        return (0, mi, seq)

    def _hichunk_merge_sort_key_seq(elem):
        seq = elem.get('chunk_seq', 0)
        tier = 1 if elem['type'] == 'table' else 0
        return (tier, seq)

    chunk_elems_only = [e for e in all_elements if e['type'] == 'chunk']
    all_chunks_have_mineru_idx = bool(chunk_elems_only) and all(
        (e.get('mineru_index') is not None and e['mineru_index'] >= 0) for e in chunk_elems_only
    )

    if all_chunks_have_mineru_idx:
        all_elements.sort(key=_hichunk_merge_sort_key_when_all_indexed)
    else:
        all_elements.sort(key=_hichunk_merge_sort_key_seq)
