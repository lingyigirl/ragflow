export default {
  translation: {
    ...
    knowledgeDetails: {
      ...
      rerankTip: `非必選項：若不選擇 rerank 模型，系統將默認採用關鍵詞相似度與向量餘弦相似度相結合的混合查詢方式；如果設定了 rerank 模型，則混合查詢中的向量相似度部分將被 rerank 打分替代。請注意：採用 rerank 模型會非常耗時。如需選用 rerank 模型，建議使用 SaaS 的 rerank 模型服務；如果你傾向使用本地部署的 rerank 模型，請務必確保你使用 docker-compose-gpu.yml 啟動 KINS。`,
      topK: 'Top-K',
...
`,
    ...
    <p>
      ...
      resume: `<p>支持的文件格式為<b>DOCX</b>、<b>PDF</b>、<b>TXT</b>。
        ...
        您只需與<i>'KINS'</i>交談即可列出所有符合資格的候選人。
        </p>
          `,
      ...
      hichunk: `<p>適用於透過 <b>MinerU</b> 解析的 <b>PDF、Excel（轉 PDF 路徑）、圖片</b> 等；解析後結合標題層級與規則完成智慧分塊，可選接入相容 OpenAI 的推理服務。</p>
      <p>請在資料集側設定嵌入模型與 PageRank；PDF 情境下可在單一文件解析對話框中選擇版面辨識與 MinerU 相關選項。</p>`,
...
以上就是你需要總結的內容。`,
 ...
 一般：實體和關係擷取提示來自 GitHub - microsoft/graphrag：基於模組化圖形的檢索增強生成 (RAG) 系統，`,
    ...
    setting: {
      ...
      embeddingModel: '嵌入模型',
      embeddingModelTip:
        '如未顯示可選模型，請檢查你是否在使用 KINS slim 版（不含嵌入模型）；或根據 https://ragflow.io/docs/dev/supported_models 確認你的模型供應商是否提供該模型。',
